...
const FeedbackCard = ({ feedback }) => {
  return (
    <article className="p-8 w-full bg-white rounded-3xl flex justify-between items-center flex-col md:flex-row gap-6">
      <span className="self-start hidden bg-slate-200/50 flex-col stroke-4 md:flex justify-center items-center p-2 w-12 rounded-xl hover:bg-slate-200/90 duration-300 hover:cursor-pointer">
      ...
      <section className="flex-1">
        ...
        <section className="rounded-2xl flex  justify-start items-start gap-4 flex-wrap mt-4">
          {feedback.categories.map((tag) => {
            return (
              <span className="py-2 px-4 rounded-xl bg-[#eaedfd] font-bold text-[#4661e6]">
                {tag}
              </span>
            );
          })}
      ...
      <section className="w-full flex justify-between items-center md:hidden ">
        <section>
          <span className="self-start bg-slate-200/50 stroke-4 flex justify-center items-center p-2 h-12 rounded-xl hover:bg-slate-200/90 duration-300 hover:cursor-pointer">
            <MdKeyboardArrowUp className="text-blue-700 text-3xl" />
